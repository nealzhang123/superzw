...
* 
*/
class Stacktech_Crm_Db {
    public $model_cus,$model_rec,$model_hol,$model_mem,$model_mail,$model_sms;//等等
    public $key_creater,$key_charger,$key_customer,$key_country,$key_province,$key_city,$key_area,$key_holiday_date;
    // DROP TABLE stacktech_2_crm_category,stacktech_2_crm_customer_info,stacktech_2_crm_form_options,stacktech_2_crm_form_option_value,stacktech_2_crm_model_area,stacktech_2_crm_member_info
	...
	function __construct() {
        ...
        $this->key_city         = 'city';
        $this->key_area         = 'area';
        $this->key_holiday_date = 'holiday_date';

	}

	function get_init_form_options() {
		$init_form_options = array(
            //客户模块 area 0
			array(
                'option_id'     => 1,
                'title'         => '客户名称',
                'form_name'     => 'customer_name',
                'form_type'     => 'text',
                'default_value' => '',
                'is_required'   => 1,
                'is_hidden'     => 0,
            ...
            array(
                ...
                'area_id'       => 0,
                'selects'       => array(
                '无','淘宝','电话来访','老客户','客户介绍','独立开发','媒体宣传','促销活动','代理商','合作伙伴','公开招标','直邮','Email群发','网站','会议','展会','口碑','其它'
                ),
                'sort'          => 8,
                'model'         => $this->model_cus,
                'creater'       => 'superadmin'
            ...
            array(
                ...
                'area_id'       => 0,
                'sort'          => 1,
                'model'         => $this->model_rec,
                'creater'       => 'superadmin'
            ),
            array(
                'option_id'     => 101,
                'title'         => '客户',
                'form_name'     => 'customer_id',
                'form_type'     => 'special',
                'default_value' => '',
                'is_required'   => 1,
                'is_hidden'     => 0,
            ...
            array(
                ...
                'area_id'       => 0,
                'selects'       => array(
                '无','拜访客户','联系小计','寄样品','询价','发报价','发货','商机转客户'
                ),
                'sort'          => 3,
                'model'         => $this->model_rec,
                'creater'       => 'superadmin'
            ...
            array(
                ...
                'area_id'       => 0,
                'selects'       => array(
                '不提醒','15分钟','30分钟','1小时','2小时','6小时','12小时','24小时'
                ),
                'sort'          => 8,
                'model'         => $this->model_rec,
                'creater'       => 'superadmin'
            ...
            array(
                ...
                'area_id'       => 0,
                'selects'       => array(
                '请选择','生日','子女生日','父母生日','结婚纪念日','公司成立纪念日','毕业纪念日','其他'
                ),
                'sort'          => 4,
                'model'         => $this->model_hol,
                'creater'       => 'superadmin'
            ...
            array(
                ...
                'sort'          => 6,
                'model'         => $this->model_hol,
                'special_key'   => $this->key_holiday_date,
                'creater'       => 'superadmin'
            ),
            array(
                'option_id'     => 2,
            ...
            //
            //联系人模块,300开头，很多option与客户模块中的联系人相同
            array(
                'option_id'     => 301,
                'title'         => '姓名',
                'form_name'     => 'member_name',
		...
		return $init_form_options;
	}

    function get_model_area_data() {
        $data = array(
            $this->model_cus => array(
                '基本信息','联系信息','地址信息','自定义信息','备注信息'
            ),
            $this->model_rec => array(
                '基本信息','自定义信息','内容信息'
            ),
            $this->model_hol => array(
                '基本信息','自定义信息','描述信息'
            ),
            $this->model_mem => array(
                '基本信息','自定义信息','描述信息'
            ),
        );
    ...
    function get_category_data() {
        $data = array(
            $this->model_cus  => array( 'cate_name' => '全部' ),
            $this->model_rec  => array( 'cate_name' => '全部' ),
            $this->model_hol  => array( 'cate_name' => '全部' ),
            $this->model_mem  => array( 'cate_name' => '全部' ),
            $this->model_mail => array( 'cate_name' => '全部' ),
            $this->model_sms  => array( 'cate_name' => '全部' ),
        );

        return $data;
    }

    function get_country_area_data() {
        $init_data = "(1, '110000', '北京市','0'),
                    (2, '120000', '天津市','0'),
                    (3, '130000', '河北省','0'),
                    (4, '140000', '山西省','0'),
                    (5, '150000', '内蒙古','0'),
        ...
        $init_data = explode( '),', $init_data );

        $data = array();
        $i = 0;
        foreach ( $init_data as $code ) {
            if( empty( $code ) )
                continue;

            $code1 = explode( '\'', $code );
            if( $code1[3] == '市辖区' || $code1[3] == '城　区' || $code1[3] == '矿　区' || $code1[3] == '郊　区' )
                continue;

            $data[$i]['code'] = $code1[1]; 
            $data[$i]['name'] = $code1[3]; 
            $data[$i]['parent'] = $code1[5]; 
            $i++;
        }
	...
	function prepare_table() {
        ...
        //通用的option表
        $form_option_value = $wpdb->prefix . 'crm_form_option_value';

        if( $wpdb->get_var( "show tables like '$form_option_value'" ) != $form_option_value ) {
            $sql = "CREATE TABLE $form_option_value  (
        ...
        //通用的area表
        $model_area = $wpdb->prefix . 'crm_model_area';

        if( $wpdb->get_var( "show tables like '$model_area'" ) != $model_area ) {
            $sql = "CREATE TABLE $model_area  (
                id int(11) NOT NULL AUTO_INCREMENT,
                area_id int(11) NOT NULL,
                area_value varchar(255),
            ...
            $this->init_model_area_data();
        }

        //通用的form_options
        $form_options = $wpdb->prefix . 'crm_form_options';

        if( $wpdb->get_var( "show tables like '$form_options'" ) != $form_options ) {
            $sql = "CREATE TABLE $form_options  (
                id int(11) NOT NULL AUTO_INCREMENT,
                option_id int(11) NOT NULL,
                title varchar(255) NOT NULL,
                form_name varchar(50) NOT NULL DEFAULT 'text',
                form_type varchar(50) NOT NULL DEFAULT 'text',
                ...
                model varchar(50) NOT NULL,
                special_key varchar(50) DEFAULT '',
                creater varchar(50),
                PRIMARY KEY  (id,option_id)
                )";
            dbDelta( $sql );
        ...
        if( $wpdb->get_var( "show tables like '$category_table'" ) != $category_table ) {
            $sql = "CREATE TABLE $category_table  (
                ...
                cate_condition text DEFAULT '',
                show_fields text DEFAULT '',
                is_pivotal tinyint NOT NULL DEFAULT 0,
                model varchar(50) NOT NULL,
                PRIMARY KEY (cate_id)
                )";
            dbDelta( $sql );
        ...
        //其他表,并初始化..
        //客户数据表
        $customer_info_table = $wpdb->prefix . 'crm_customer_info';
        ...
        if( $wpdb->get_var( "show tables like '$customer_info_table'" ) != $customer_info_table ) {
            $sql = "CREATE TABLE $customer_info_table  (
                ...
                customer_status smallint NOT NULL DEFAULT 0,
                user_id smallint NOT NULL,
                vip int(11) NOT NULL DEFAULT 0,
                tradedwish smallint NOT NULL DEFAULT 0,
                fever smallint NOT NULL DEFAULT 0,
                customer_from smallint NOT NULL DEFAULT 0,
        ...
        //联系人数据表
        $member_info_table = $wpdb->prefix . 'crm_member_info';
        
        if( $wpdb->get_var( "show tables like '$member_info_table'" ) != $member_info_table ) {
            $sql = "CREATE TABLE $member_info_table  (
        ...
        //联系人数据表
        $record_info_table = $wpdb->prefix . 'crm_record_info';
        
        if( $wpdb->get_var( "show tables like '$record_info_table'" ) != $record_info_table ) {
            $sql = "CREATE TABLE $record_info_table  (
        ...
        //纪念日数据表
        $holiday_info_table = $wpdb->prefix . 'crm_holiday_info';
        
        if( $wpdb->get_var( "show tables like '$holiday_info_table'" ) != $holiday_info_table ) {
            $sql = "CREATE TABLE $holiday_info_table  (
                holiday_id int(11) NOT NULL AUTO_INCREMENT,
                topic varchar(255) NOT NULL,
                user_id int(11) NOT NULL,
                customer_id int(11) NOT NULL,
                member_id int(11) DEFAULT 0,
                holiday_type smallint NOT NULL DEFAULT 0,
                calendar_type smallint NOT NULL DEFAULT 0,
        ...
        //发送邮件数据表
        $mail_info_table = $wpdb->prefix . 'crm_mail_info';
        
        if( $wpdb->get_var( "show tables like '$mail_info_table'" ) != $mail_info_table ) {
            $sql = "CREATE TABLE $mail_info_table  (
                mail_id int(11) NOT NULL AUTO_INCREMENT,
                mail_group int(11) NOT NULL,
                mail_topic varchar(255) NOT NULL,
                user_id int(11) NOT NULL,
                user_email varchar(255),
                recipient_email varchar(255),
                recipient_name varchar(255),
                mail_content text,
                mail_status smallint NOT NULL DEFAULT 0,
                mail_time datetime,
                is_delete tinyint DEFAULT 0,
                PRIMARY KEY  (mail_id,user_id,mail_status)
                )";
            dbDelta( $sql );
        ...
        //发送短信数据表
        $sms_info_table = $wpdb->prefix . 'crm_sms_info';
        
        if( $wpdb->get_var( "show tables like '$sms_info_table'" ) != $sms_info_table ) {
            $sql = "CREATE TABLE $sms_info_table  (
                sms_id int(11) NOT NULL AUTO_INCREMENT,
                sms_group int(11) NOT NULL,
                sms_content varchar(255) NOT NULL,
                user_id int(11) NOT NULL,
                recipient_phone int(11),
                recipient_name varchar(255),
                sms_status smallint NOT NULL DEFAULT 0,
                sms_public_time datetime,
                sms_time datetime,
                is_delete tinyint DEFAULT 0,
                PRIMARY KEY  (sms_id,user_id,sms_status)
                )";
            dbDelta( $sql );
        ...
        //国内省市区代码表
        $country_area_table = $wpdb->base_prefix . 'country_area_list';
        
        if( $wpdb->get_var( "show tables like '$country_area_table'" ) != $country_area_table ) {
            $sql = "CREATE TABLE $country_area_table  (
                id int(11) NOT NULL AUTO_INCREMENT,
                code int(11) NOT NULL,
                name varchar(255) NOT NULL,
                parent int(11) NOT NULL,
                PRIMARY KEY  (id,code,name,parent)
                )";
            dbDelta( $sql );

            $this->init_country_area_data();
        }

        //邮件，短信模板表
        $send_viewer_table = $wpdb->prefix . 'crm_send_viewer';
        
        if( $wpdb->get_var( "show tables like '$send_viewer_table'" ) != $send_viewer_table ) {
            $sql = "CREATE TABLE $send_viewer_table  (
    ...
    function init_country_area_data() {
        global $wpdb;

        $data = $this->get_country_area_data();

        foreach ($data as $item) {
            $country_area_table = $wpdb->base_prefix . 'country_area_list';

            $wpdb->insert( $country_area_table, $item );
        }
    }

    function init_model_area_data() {
        $data = $this->get_model_area_data();

        foreach ( $data as $model => $areas ) {
            foreach ( $areas as $area_id => $area_value ) {
                $area = array(
                    'area_id' => $area_id,
                    'area_value' => $area_value,
                    'model' => $model
                    );
                $id = $this->add_model_single_area( $area );

                $this->update_model_single_area( array( 'area_sort' => $id ), array( 'id' => $id ) );
            }
        }
    ...
    function add_model_single_area( $area ) {
        global $wpdb;

        $model_area = $wpdb->prefix . 'crm_model_area';

        $wpdb->insert( $model_area, $area );

        return $wpdb->insert_id;
    }

    function update_model_single_area( $data, $where ) {
        global $wpdb;

        $model_area = $wpdb->prefix . 'crm_model_area';

        $wpdb->update( $model_area, $data, $where );

        return;
    }

    function init_form_data() {
    	$data = $this->get_init_form_options();

        $repeat = array();
    	foreach ($data as $option) {
    		$this->add_form_options( $option );
            //$this->update_customer_form_option( array( 'sort' => $option_id ), array( 'option_id' => $option_id ) );

    		if( array_key_exists( 'selects', $option ) && !in_array( $option['option_id'], $repeat ) ) {
    			$this->add_form_option_values( $option['option_id'], $option['selects'] );
                $repeat[] = $option['option_id'];
    		}
    	}
    ...
    function add_form_options( $option ) {
    	global $wpdb;

    	$form_options = $wpdb->prefix . 'crm_form_options';
    	if( array_key_exists( 'selects', $option ) )
    		unset( $option['selects'] );

    	$wpdb->insert( $form_options, $option );

    	return $wpdb->insert_id;
    }

    function update_form_option( $data, $where ) {
        global $wpdb;

        $form_options = $wpdb->prefix . 'crm_form_options';

        $wpdb->update( $form_options, $data, $where );

        $data_2['title'] = $data['title'];
        $where_2['option_id'] = $where['option_id'];

        $wpdb->update( $form_options, $data_2, $where_2 );

        return;
    }

    function add_form_option_values( $option_id, $selects ) {
    	if( !is_array( $selects ) || count( $selects ) < 1 )
    		return;

    	foreach ( $selects as $key => $value ) {
    		$select = array();
    		$select['option_id'] = $option_id;
    		$select['option_key'] = $key;
    		$select['option_value'] = $value;

    		$this->add_single_form_option_value( $select );
    	}

        return;
    }

    function add_single_form_option_value( $data ) {
    	global $wpdb;

    	$form_option_value = $wpdb->prefix . 'crm_form_option_value';

    	$wpdb->insert( $form_option_value, $data );

    	return $wpdb->insert_id;
    }

    function get_form_option_by_form_type( $model, $form_type ) {
    	global $wpdb;

    	$table = $wpdb->prefix . 'crm_form_options';
    	$sql = "SELECT * FROM " . $table . " WHERE form_type='" . $form_type . "' AND model='" . $model . "' ORDER BY sort ASC";

    	$results = $wpdb->get_results( $sql, 'ARRAY_A' );

        foreach ( $results as $key => $item ) {
            $results[$key]['options'] = $this->get_form_option_values_by_id( $item['option_id'] );
        }

    	return $results;
    }

    function get_form_option( $model, $area_id ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_form_options';
        $sql = "SELECT * FROM " . $table . " WHERE area_id=" . $area_id . " AND model='" . $model . "' ORDER BY sort ASC";
    ...
    function get_form_option_values_by_id( $option_id ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_form_option_value';

        $sql = "SELECT * FROM " . $table . " WHERE option_id=" . $option_id;
        $results = $wpdb->get_results( $sql, 'ARRAY_A' );

        return $results;
    }

    function update_option_values_by_option_id( $option_id, $option_values ) {

        $this->delete_form_option_by_option_id( $option_id );
        $this->add_form_option_values( $option_id, $option_values );

        return;
    }

    function delete_form_option_by_option_id( $option_id ) {
        global $wpdb;
    ...
    function get_model_areas( $model ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_model_area';

        $sql = "SELECT * FROM " . $table . " WHERE model='" . $model . "' ORDER BY area_sort ASC";
        $results = $wpdb->get_results( $sql, 'ARRAY_A' );
    ...
    function init_category_data() {
        ...
        foreach ( $data as $key => $value ) {
            $item['model'] = $key;
            $item['cate_name'] = $value['cate_name'];

            $this->add_category( $item );
        }
    ...
    function get_all_categories( $model ) {
        global $wpdb;

        $category_table = $wpdb->prefix . 'crm_category';

        $sql = "SELECT * FROM " . $category_table . " WHERE model='" . $model . "'";
        $results = $wpdb->get_results( $sql, 'ARRAY_A' );
    ...
    //for customer_info
    function add_crm_customer_info( $data ) {
        global $wpdb;
    ...
    function update_crm_customer_info( $data, $id ) {
        global $wpdb;
    ...
    function get_crm_customer_info_by_id( $id ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_customer_info';

        $sql = "SELECT * FROM " . $table . " WHERE customer_id=" . $id . " AND is_delete=0";
        $results = $wpdb->get_row( $sql, 'ARRAY_A' );

        return $results;
    }

    function get_crm_customers_name() {
        global $wpdb;
        ...
        if( count( $results ) ) {
            foreach ($results as $key => $item) {
                $results[$key]['option_key'] = $item['customer_id'];
                $results[$key]['option_value'] = $item['customer_name'];
            } 
        }
    ...
    //for member_info
    function add_crm_member_info( $data ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_member_info';
    ...
    function update_crm_member_info( $data, $id ) {
        global $wpdb;
    ...
    function get_crm_members_by_customer_id( $id ) {
        global $wpdb;
    ...
    function get_crm_member_by_member_id( $id ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_member_info';

        $sql = "SELECT * FROM " . $table . " WHERE member_id=" . $id . " AND is_delete=0";
        $results = $wpdb->get_row( $sql, 'ARRAY_A' );

        return $results;
    }

    function delete_crm_member_by_customer( $id ) {
        global $wpdb;
    ...
    function get_email_addresses_by_type( $type, $seleted ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_member_info';

        if( $type == 'customer' ) {
            $sql = 'SELECT * FROM ' . $table . ' WHERE customer_id IN (' . $seleted . ') AND is_delete=0';
        }elseif( $type = 'member' ) {
            $sql = 'SELECT * FROM ' . $table . ' WHERE member_id IN (' . $seleted . ') AND is_delete=0';
        }
        $results = $wpdb->get_results( $sql , 'ARRAY_A' ); 
    ...
    //for record table
    function add_crm_record_info( $data ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_record_info';
    ...
    function update_crm_record_info( $data, $id ) {
        global $wpdb;
    ...
    function get_crm_record_by_record_id( $id ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_record_info';

        $sql = "SELECT * FROM " . $table . " WHERE record_id=" . $id . " AND is_delete=0";
        $results = $wpdb->get_row( $sql, 'ARRAY_A' );

        return $results;
    }

    function delete_crm_record( $id ) {
        global $wpdb;
    ...
    //for holiday table
    function add_crm_holiday_info( $data ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_holiday_info';
    ...
    function update_crm_holiday_info( $data, $id ) {
        global $wpdb;
    ...
    function get_crm_holiday_by_holiday_id( $id ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_holiday_info';

        $sql = "SELECT * FROM " . $table . " WHERE holiday_id=" . $id . " AND is_delete=0";
        $results = $wpdb->get_row( $sql, 'ARRAY_A' );

        return $results;
    }

    function delete_crm_holiday( $id ) {
        global $wpdb;
    ...
    //for mail
    function add_crm_mail_info( $data ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_mail_info';
    ...
    function delete_crm_mail( $id ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_mail_info';

        $data = array( 'is_delete' => 1 );
        $where = array( 'mail_group' => $id );
    ...
    //for sms
    function add_crm_sms_info( $data ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_sms_info';
    ...
    function delete_crm_sms( $id ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_sms_info';

        $data = array( 'is_delete' => 1 );
        $where = array( 'sms_group' => $id );

        $wpdb->update( $table, $data, $where );
    }

    function get_week_record( $user_id ) {
        global $wpdb;
        ...
        $sql = "SELECT * FROM " . $table . " WHERE next_contact_time BETWEEN '" . $start . "' AND '" . $end . "'";
        if( !user_can( $user_id, 'manage_options' ) )
            $sql.= " AND user_id=" . $user_id;

        $sql .= " ORDER BY next_contact_time ASC";
        $results = $wpdb->get_results( $sql, 'ARRAY_A' );
    ...
    //for province list
    //
    function get_crm_provinces() {
        global $wpdb;

        $table = $wpdb->base_prefix . 'country_area_list';
        ...
        foreach ($results as $key => $value) {
           $results[$key]['option_key'] = $value['code'];
           $results[$key]['option_value'] = $value['name'];
        }

        return $results;
    }

    function get_data_by_parent( $id ) {
        global $wpdb;
    ...
    //for send viewer
    //
    function update_send_viewer( $data ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_send_viewer';

        if( empty( $data['viewer_id'] ) ) {
            ...
            $where = array( 'viewer_id' => $data['viewer_id'] );
    ...
    function get_send_viewer_by_id( $id ) {
        global $wpdb;

        $table = $wpdb->prefix . 'crm_send_viewer';

        $sql = "SELECT * FROM " . $table . " WHERE viewer_id=" . $id;
    ...
    function get_all_viewers( $model ) {
        global $wpdb;
