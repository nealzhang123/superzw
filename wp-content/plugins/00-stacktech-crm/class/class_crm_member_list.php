...
class Crm_Member_List extends WP_List_Table {
    public $cate_id;
	...
	function get_member_count(){
        ...
        if( empty( $this->cate_id ) ) {
            $member_table = $wpdb->prefix . 'crm_member_info';
            $sql = "SELECT count(*) FROM " . $member_table . " WHERE is_delete=0";
	...
	function get_member_infos( $per_page = 10 ,$page = 1 ) {
		global $wpdb;

        if( empty( $this->cate_id ) ) {
            $member_table = $wpdb->prefix . 'crm_member_info';
            $sql = "SELECT * FROM " . $member_table . " WHERE is_delete=0";
    ...
    function prepare_items() {
        ...
        $per_page = 10;
        $total_count = $this->get_member_count();
        ...
        $this->items = $this->get_member_infos();
    ...
    function column_default( $item, $column_name ){
        ...
        $sql = "SELECT option_id,form_type,special_key FROM " . $form_options . " WHERE model='member' AND form_name='" . $column_name . "'";

        $member = $wpdb->get_row( $sql, 'ARRAY_A' );

        switch ( $member['form_type'] ) {
            case 'select':
                $form_values = $wpdb->prefix . 'crm_form_option_value';
                $sql = "SELECT option_value FROM " . $form_values . " WHERE option_id=" . $member['option_id'] . " AND option_key='" . $item[$column_name] . "'";
    ...
    function column_member_name( $item ) {
        echo '<a href="'.admin_url( 'admin.php?page=crm_edit_model&model=member&pid='.$item['member_id'] ).'" target="_blank">'.$item['member_name'].'</i></a>';
    }
    ...
    function get_columns() {
        ...
        if( empty( $this->cate_id ) ) {
            $form_options = $wpdb->prefix . 'crm_form_options';
            $sql = "SELECT * FROM " . $form_options . " WHERE model='member'";

            $member_form = $wpdb->get_results( $sql, 'ARRAY_A' );
        }else{
            $category_table = $wpdb->prefix . 'crm_category';
    ...
    function column_tools( $item ) {
        echo '<a href="'.admin_url( 'admin.php?page=crm_edit_model&model=member&pid='.$item['member_id'] ).'" target="_blank"><i class="fa fa-pencil-square-o"></i></a>';
        echo '&nbsp;|&nbsp;';
        echo '<a class="list_item_delete" data-pid="'.$item['member_id'].'"><i class="fa fa-trash-o"></i></a>';
    }

    function column_cb( $item ) {
        return sprintf('<input type="checkbox" name="bulk-delete[]" value="%s" />', $item['member_id']);
    }
