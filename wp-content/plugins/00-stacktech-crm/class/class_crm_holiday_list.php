...
class Crm_Holiday_List extends WP_List_Table {
    public $cate_id;
	...
	function get_holiday_count(){
        ...
        if( empty( $this->cate_id ) ) {
            $holiday_table = $wpdb->prefix . 'crm_holiday_info';
            $sql = "SELECT count(*) FROM " . $holiday_table . " WHERE is_delete=0";
	...
	function get_holiday_infos( $per_page = 10 ,$page = 1 ) {
		global $wpdb;

        if( empty( $this->cate_id ) ) {
            $holiday_table = $wpdb->prefix . 'crm_holiday_info';
            $sql = "SELECT * FROM " . $holiday_table . " WHERE is_delete=0";

            $holiday = $wpdb->get_results( $sql, 'ARRAY_A' );
        }else{
            $category_table = $wpdb->prefix . 'crm_category';
            $sql = "SELECT cate_condition FROM " . $category_table . " WHERE cate_id=" . $this->cate_id;
            $condition = $wpdb->get_val( $sql );
    ...
    function prepare_items() {
        ...
        $per_page = 10;
        $total_count = $this->get_holiday_count();
        ...
        $this->items = $this->get_holiday_infos();
    ...
    function column_default( $item, $column_name ){
        ...
        $sql = "SELECT option_id,form_type,special_key FROM " . $form_options . " WHERE model='holiday' AND form_name='" . $column_name . "'";

        $holiday = $wpdb->get_row( $sql, 'ARRAY_A' );

        switch ( $holiday['form_type'] ) {
            case 'select':
                $form_values = $wpdb->prefix . 'crm_form_option_value';
                $sql = "SELECT option_value FROM " . $form_values . " WHERE option_id=" . $holiday['option_id'] . " AND option_key='" . $item[$column_name] . "'";
    ...
    function column_topic( $item ) {
        echo '<a href="'.admin_url( 'admin.php?page=crm_edit_model&model=holiday&pid='.$item['holiday_id'] ).'" target="_blank">'.$item['topic'].'</i></a>';
    }
    ...
    function get_columns() {
        ...
        if( empty( $this->cate_id ) ) {
            $form_options = $wpdb->prefix . 'crm_form_options';
            $sql = "SELECT * FROM " . $form_options . " WHERE model='holiday'";

            $holiday_form = $wpdb->get_results( $sql, 'ARRAY_A' );
        }else{
            $category_table = $wpdb->prefix . 'crm_category';
        ...
        foreach ($holiday_form as $option) {
            if( $i == 10 )
                break;
    ...
    function column_tools( $item ) {
        echo '<a href="'.admin_url( 'admin.php?page=crm_edit_model&model=holiday&pid='.$item['holiday_id'] ).'" target="_blank"><i class="fa fa-pencil-square-o"></i></a>';
        echo '&nbsp;|&nbsp;';
        echo '<a class="list_item_delete" data-pid="'.$item['holiday_id'].'"><i class="fa fa-trash-o"></i></a>';
    }
